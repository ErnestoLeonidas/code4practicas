...
**Built through v0.5.0** (see `CHANGELOG.md`): full-stack scaffolding (`api/` + `frontend/`), SQLite-backed schema, authentication (login/logout/me with PHP sessions + route guard), admin user management (CRUD `/api/usuarios`, system-generated passwords, forced first-login password change, role-restricted routes), password recovery flow (forgot-password → email link → reset; PHPMailer with HTML templates), and student/career management (CRUD `pp_carreras` + `pp_estudiantes`, RUT validation, role-scoped filtering), and company/supervisor management (CRUD `pp_empresas` + `pp_supervisores`, supervisores embedded in empresa detail, migration `003` adds `activo` to both tables). Not yet committed to git. `ROADMAP.md` is the authoritative spec and build plan; read the relevant version section before building the next one. Do not invent architecture that contradicts it.
...
Contracts: `GET /api/health`. Auth: `POST /api/auth/login {correo,password}`, `POST /api/auth/logout`, `GET /api/auth/me`, `POST /api/auth/cambiar-password {password_actual,password_nueva}`, `POST /api/auth/recuperar {correo}` (always-200, no leak), `POST /api/auth/restablecer {token,password_nueva}` (session cookie `pp_sesion`, HttpOnly + SameSite=Lax). Admin-only (RoleMiddleware `permitir('admin')`): `GET/POST/PUT/DELETE /api/usuarios` (paginated `?page=&per_page=&q=&rol=&activo=`, logical delete), `POST /api/usuarios/{id}/regenerar-password`. Passwords are always system-generated (`Password::generar`), returned once as `password_generada`; email delivery via `Services/Mailer` (full PHPMailer with HTML templates, `enviarCredenciales`+`enviarRecuperacion`; returns false without SMTP config). Recovery tokens: `bin2hex(random_bytes(32))` → only HMAC-SHA256 stored in `pp_tokens_recuperacion`; TTL 60 min; keyed by `Config::get('app_secret')`; `Token` model in `src/Models/Token.php`. Careers: `GET /api/carreras` (no pagination), `POST/PUT/DELETE /api/carreras` (admin-only; 409 `carrera_en_uso` on delete with active students). Students: `GET/POST/PUT/DELETE /api/estudiantes` (admin+docente; docente auto-filtered to own `docente_id`); RUT validated with Chilean DV algorithm (`Validaciones::rutValido()`); semestre format `AAAA-[12]`; `Carrera` and `Estudiante` models in `src/Models/`. Seed: `php api/seed_carreras.php` (13 Duoc UC careers, idempotent). Error codes so far: `datos_invalidos`, `dominio_no_institucional`, `credenciales_invalidas`, `demasiados_intentos`, `no_autenticado`, `sin_permiso`, `correo_duplicado`, `rol_invalido`, `no_encontrado`, `no_puede_desactivarse`, `ultimo_admin`, `password_actual_incorrecta`, `password_debil`, `token_invalido`, `rut_invalido`, `rut_duplicado`, `semestre_invalido`, `carrera_invalida`, `docente_invalido`, `carrera_en_uso`, `url_invalida`, `correo_invalido`, `empresa_en_uso`.
